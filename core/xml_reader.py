...
from .bookmerger import BookMerger, BookPriceType
...
class XmlReader:
    ...
    def iter_rows_data(self, file_or_url: str):
        if file_or_url.startswith("http"):
            file_name = self.get_file_from_url(file_or_url)
        else:
        ...
        try:
            ...
            logger.info(f"XmlReader: Cannot open file {file_name} as XML: {e}")
            return False
        root = tree.getroot()
        ...
        for child in root[:10]:
            if child.tag.lower() in self.children_node_names:
                ...
                if child_counter >= 2:
                    # Bingo!
                    logger.info(f"XmlReader: Parent of children is root: {root}")
        ...
        if not children_parent:
            ...
            child_counter = 0
            for child in root[:10]:
                for item in child[:10]:
                    if item.tag.lower() in self.children_node_names:
                        child_counter += 1
                        if child_counter >= 2:
                            # Bingo!
                            logger.info(f"XmlReader: Parent of children id child of root: {child}")
        ...
        if not children_parent:
            logger.warning("XmlReader: Children parent not found!")
            return False

        for item in children_parent:
            if item.tag.lower() in self.children_node_names:
    ...
    def get_file_from_url(self, import_url: str) -> int:
